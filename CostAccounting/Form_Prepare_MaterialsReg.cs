...
{
    public partial class Form_Prepare_MaterialsReg : Form
    {
        public Form_Prepare_MaterialsReg()
        {
            InitializeComponent();
        }
         ...
         * フォームロード時の処理
         *************************************************************/
        private void Form_Prepare_MaterialsReg_Load(object sender, EventArgs e)
        {
        ...
        private void btnAppend_Click(object sender, EventArgs e)
        {
            if (Program.MessageBoxBefore("登録しますか？") == DialogResult.Yes)
            {
                ...
                using (var context = new CostAccountingEntities())
                {
                    ...
                    {
                        Program.MessageBoxError("既に同じコードの資材が登録されています。");
                    }
            ...
            if (Program.MessageBoxBefore("削除しますか？") == DialogResult.Yes)
            {
                using (var context = new CostAccountingEntities())
                {
                    ...
                    if (target.Count() > 0)
                    {
                        ...
                        context.Material.Remove(target.First());
                    }
                    context.SaveChanges();
                }
        ...
        private void btnFileReg_Click(object sender, EventArgs e)
        {
            ...
            if (Program.MessageBoxBefore("登録しますか？" + Environment.NewLine + "※現在登録されているデータは削除されます※") == DialogResult.Yes)
            {
                using (var context = new CostAccountingEntities())
                {
                    ...
                    context.Material.RemoveRange(target);
                ...
                Logger.Info(Message.INF003, new string[] { this.Text, Message.create(labelFilePath, recordCnt) });
                Program.MessageBoxAfter("登録し、再検索を行いました。");
                btnSearch_Click(sender, e);
            }
        }
