...
{
    public partial class Form_Prepare_SupplierReg : Form
    {
        public Form_Prepare_SupplierReg()
        {
            InitializeComponent();
        }
         ...
         * フォームロード時の処理
         *************************************************************/
        private void Form_Prepare_SupplierReg_Load(object sender, EventArgs e)
        {
            using (var context = new CostAccountingEntities())
            {
                var list = from t in context.Supplier
        ...
        private void btnAppend_Click(object sender, EventArgs e)
        {
            if (Program.MessageBoxBefore("登録しますか？") == DialogResult.Yes)
            {
                ...
                using (var context = new CostAccountingEntities())
                {
                    var target = from t in context.Supplier
            ...
            if (Program.MessageBoxBefore("削除しますか？") == DialogResult.Yes)
            {
                using (var context = new CostAccountingEntities())
                {
                    ...
                    if (target.Count() > 0)
                    {
                        ...
                        context.Supplier.Remove(target.First());
                    }
                    context.SaveChanges();
                }
        ...
        private void btnFileReg_Click(object sender, EventArgs e)
        {
            ...
            if (Program.MessageBoxBefore("登録しますか？" + Environment.NewLine + "※現在登録されているデータは削除されます※") == DialogResult.Yes)
            {
                using (var context = new CostAccountingEntities())
                {
                    ...
                    context.Supplier.RemoveRange(target);
